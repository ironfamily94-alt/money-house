...
$("#nw-copy").onclick=()=>{
  ...
  // 지난 달 '항목 이름 + 금액'을 가져오되, 이번 달에 이미 입력한 건 절대 덮어쓰지 않고 그대로 둠 (없는 항목만 추가)
  let added=0;
  nwAll().forEach(c=>{ const existing=dst[c]||(dst[c]=[]);
    ...
    (src[c]||[]).forEach(it=>{ const nm=(it.name||"").trim();
      if(nm && !names.has(nm)){ existing.push({name:it.name,amount:Number(it.amount)||0}); names.add(nm); added++; } }); });
  saveNetworthNow(); renderNetworth();
  alert(added>0 ? ("지난 달 항목과 금액 "+added+"개를 불러왔어요.\n(이번 달에 이미 입력한 숫자는 그대로 있고, 새로 온 금액은 필요하면 고치세요 👍)")
